...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const drawGradientOrbs = () => {
      ...
      // Draw animated orbs
      const orbs = [
        { x: canvas.width * 0.2, y: canvas.height * 0.3, color: 'rgba(107, 159, 127, 0.15)' },
        { x: canvas.width * 0.8, y: canvas.height * 0.7, color: 'rgba(201, 115, 92, 0.1)' },
        { x: canvas.width * 0.5, y: canvas.height * 0.2, color: 'rgba(45, 80, 22, 0.08)' }
      ]
    ...
    <section className="relative min-h-screen pt-32 pb-20 px-4 flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-5xl mx-auto text-center">
        <div className="mb-8 inline-block">
          <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-wellness-sage/10 border border-wellness-sage/30 text-wellness-moss text-sm font-semibold">
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-wellness-sage opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-wellness-sage"></span>
            </span>
            AI-Powered Wellness Platform
          </span>
        </div>

        <h1 className="text-7xl md:text-8xl font-serif font-bold text-wellness-dark mb-6 leading-tight">
          Your Wellness<br />
          <span className="bg-gradient-to-r from-wellness-moss to-wellness-sage bg-clip-text text-transparent">
            Journey Awaits
          </span>
        </h1>
        ...
        <div className="grid grid-cols-3 gap-8 text-center max-w-3xl mx-auto">
          <div className="group cursor-pointer">
            <div className="text-4xl md:text-5xl font-serif font-bold text-wellness-sage mb-2 group-hover:text-wellness-moss transition">
              10K+
            </div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-10">
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="w-6 h-10 border-2 border-wellness-sage rounded-full flex items-start justify-center p-2">
            <div className="w-1 h-2 bg-wellness-sage rounded-full animate-bounce"></div>
          </div>
        </div>
      </div>
    </section>
